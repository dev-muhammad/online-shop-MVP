...
<div class="z-shop-cart">
  ...
  <template>
    <div class="text-center">
        <v-bottom-sheet v-model="cartSheet" style="overflow-y:auto">
        <v-list style="position:relative;border-radius:10px 10px 0px 0px">
            ...
            <div  v-show="cartItems.length" style="position:absolute;top:15px;right:15px;">
                <v-btn class="ma-0" dark depressed small color="#008dd2" 
                  nuxt to="/checkout"
                  :loading="loadingBtn"
                  @click="loadingBtn=true"
            ...
            <div class="z-bottom-cart-sheet px-2">
                ...
                <zCartItem v-for="(item,index) in cartItems" :key=index :product=item :index=index @remove=remove @editParams=editParams></zCartItem>
            </div>
        </v-list>
        </v-bottom-sheet>
        </div>
